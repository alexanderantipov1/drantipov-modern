...
export default function CalculatorPackages({
   ...
   *
   * Grid mode only. Embedded (initialSlug) blocks never reposition the page,
   * so taking scroll restoration away there would silently break Back on every
   * article that carries the calculator - the visitor would land at the top of
   * the page instead of where they left off.
   */
  useEffect(() => {
    if (lockedToPackage) return;
  ...
  useEffect(() => {
    /* Embedded mode has no grid to return to - leave Back alone. */
    if (lockedToPackage) return;
  ...
  }, [active, lockedToPackage, tracksHistory, scrollToTop]);
