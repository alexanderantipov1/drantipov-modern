...
/**
 * True when the RU dictionary has an entry for this id/field.
 *
 * Exists for `scripts/check-calc-i18n.mjs`: the dictionary is keyed by
 * DentalPrice ids, so anything newly created in their dashboard falls back to
 * English on /ru silently (by design - never blank, never a crash). The audit
 * script walks the live funnel and fails when a field has no RU entry yet, so
 * the gap surfaces before deploy instead of on a patient's screen.
 *
 * Lookup order is identical to translateCalcField(): composite key first
 * (`id::english text`, for ids reused across packages with different copy),
 * then plain id.
 */
export function hasRuTranslation(
  id: string,
  discriminator: string,
  field: "name" | "description",
): boolean {
  const composite = DICT[`${id}::${discriminator.trim()}`];
  if (composite && composite[field] != null) return true;
  const byId = DICT[id];
  return !!(byId && byId[field] != null);
}
